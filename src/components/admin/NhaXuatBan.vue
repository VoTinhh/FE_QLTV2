                    ...
                    <table class="table table-bordered table-hover align-middle shadow-sm">
                        ...
                        <tbody>
                            <tr v-for="(value, index) in list" :key="index">
                                ...
                                <td class="align-middle text-wrap">{{ value.ten_nha_xuat_ban }}</td>
                                <td class="text-center">
                                    
                                    <button class="btn btn-sm btn-primary me-2 shadow-sm" data-bs-toggle="modal"
                                            data-bs-target="#editModal" @click="edit_nha_xuat_ban = { ...value }">
                                            <i class="fas fa-edit"></i> Chỉnh Sửa
                                        </button>
                                        <button class="btn btn-sm btn-danger shadow-sm" data-bs-toggle="modal"
                                            data-bs-target="#deleteModal" @click="delete_nha_xuat_ban = { ...value }">
                    ...
                    <form>
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input v-model="create_nha_xuat_ban.ten_nha_xuat_ban" type="text" class="form-control">
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    <form>
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <label for="nameInput" class="form-label">Tên Nhà Xuất Bản</label>
                                <input v-model="edit_nha_xuat_ban.ten_nha_xuat_ban" type="text" class="form-control">
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        layData() {
            axios.get("http://127.0.0.1:8000/api/nha-xuat-ban/data").then((res) => {
                this.list = res.data.data;
            });
        },
        themMoi() {
            axios.post("http://127.0.0.1:8000/api/nha-xuat-ban/create", this.create_nha_xuat_ban).then((res) => {
                if (res.data.status) {
        ...
        capnhat() {
            axios.put(`http://127.0.0.1:8000/api/nha-xuat-ban/${this.edit_nha_xuat_ban.id}`, this.edit_nha_xuat_ban).then((res) => {
                if (res.data.status) {
        ...
        xoa() {
            axios
                .delete(`http://127.0.0.1:8000/api/nha-xuat-ban/delete/${this.delete_nha_xuat_ban.id}`)
                .then((res) => {
